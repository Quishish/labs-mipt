...
# Набор 1
x1 = np.array([1, 2, 3])
y1 = np.array([55, 142, 227])
sigma1 = np.ones_like(y1) * 2

# Набор 2
x2 = np.array([1,2,3])
y2 = np.array([41, 110, 180])
sigma2 = np.ones_like(y2) * 2

# Набор 3
x3 = np.array([1,2,3,4])
y3 = np.array([32,91,148,203])
sigma3 = np.ones_like(y3) * 2 

# Набор 4
x4 = np.array([1,2,3,4])
y4 = np.array([25,72,122,170])
...
datasets = [
    ...
    (x3, y3, sigma3, '3 кГц', 'green'),
    (x4, y4, sigma4, '3ю5 кГц', 'purple')
]
...
MU_O2 = 0.032  # Молярная масса кислорода, кг/моль
GAMMA_THEORY_O2 = 1.40  # Теоретическое значение для двухатомного газа
TEMPERATURE = 293  # Температура газа, К (20°C)
TEMP_ERROR = 1  # Погрешность температуры, К

# ЧАСТОТЫ ДЛЯ O2 (в порядке возрастания номеров наборов): 2, 2.5, 3, 3.5 кГц
FREQUENCIES_O2 = np.array([2000, 2500, 3000, 3500])  # Гц
...
# ============================================================================
print("=" * 80)
print("ОБРАБОТКА ДАННЫХ: КИСЛОРОД (O₂)")
print("=" * 80)
print(f"Температура: {TEMPERATURE} ± {TEMP_ERROR} К")
print(f"Молярная масса: {MU_O2} кг/моль")
print(f"Теоретическое γ: {GAMMA_THEORY_O2}")
print("=" * 80)
...
for i, (x, y, sigma, label, color) in enumerate(datasets):
    ...
    epsilon = abs(gamma - GAMMA_THEORY_O2) / GAMMA_THEORY_O2 * 100
...
print(f"Теоретическое значение: {GAMMA_THEORY_O2}")
print(f"Относительное отклонение: {abs(gamma_mean - GAMMA_THEORY_O2)/GAMMA_THEORY_O2*100:.2f}%")
...
plt.title('Зависимость удлиннения трубы от номера резонанса (O₂)', fontsize=14)

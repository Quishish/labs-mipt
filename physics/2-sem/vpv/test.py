...
# Если есть вторые измерения (с _2), их можно добавить отдельно
# Например:
# data_list.extend([(11.0, "10_2(11).csv"), (22.0, "20_2(22).csv"), ...])

def exp_decay(t, U0, tau):
    return U0 * np.exp(-t / tau)

def process_and_plot(P, fname):
    """Обрабатывает один файл и строит два графика, возвращает tau и Dn"""
    ...
    plt.tight_layout()
    # Сохраняем рисунок
    plt.savefig(f'fit_P{P}.png', dpi=150)
    plt.close()  # закрываем, чтобы не засорять экран (можно заменить на plt.show() если нужно смотреть)
    
    # Печать параметров
    print(f"Файл {fname}: P={P} торр, τ={tau_fit:.1f}±{tau_err:.1f} с, R²={r2:.4f}, Dn={Dn:.5f}±{Dn_err:.5f} см²/с")
